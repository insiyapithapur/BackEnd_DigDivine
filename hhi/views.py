...
class IsAuthenticatedUser(BasePermission):
    def has_permission(self, request, view):
        if request.method in ['GET','POST', 'PUT', 'PATCH', 'DELETE']:
            ...
            return bool(
                authenticate
                and HHIUser.objects.filter(
                    user=request.user, hhi_user=True
                ).exists()
            )
        return True  # Allow other HTTP methods without checks
...
class UserAuthentication(APIView):
    ...
    def _extracted_from_post_7(self, mob, user_mob_id):
        hhi_user = HHIUser.objects.get(mob=mob)
        if(hhi_user.hhi_user == False):
        ...
        if(hhi_user.user_mob_id != user_mob_id):
        ...
        if mob == "+916370067211" or hhi_user.user.is_staff == True:
            return Response({"status":"ok","message":"You are a staff user, provide your potp"}, status=status.HTTP_200_OK)
        sendotp(mob, hhi_user.user)
    ...
    def post(self, request, form=None):
        try:
            with transaction.atomic():
                ...
                if(mob is None or len(mob) < 12):
                    return Response({"status": "failed","message":"Invalid mobile number!"}, status=status.HTTP_400_BAD_REQUEST)
                hhi_user = HHIUser.objects.filter(user_mob_id=user_mob_id)
                AddLogs(None, 'UserRegistration', f"hhi_user - {hhi_user}, SignupData -{signup_data}")
                if (len(hhi_user) > 0 and hhi_user.exists()):
                ...
                if user_exist := User.objects.filter(mob=mob).exists():
                    if hhi_user := HHIUser.objects.filter(user= User.objects.get(mob=mob)).exists():
...
class verifyOtp(APIView):
    ...
    def new_user_registration(self, mob, otp):
        ...
        if(check_pre_signup.exists()):
            return FinalRegistration(PreSignUP,check_pre_signup.first().signup_token, HHIUser, AddLogs)
    ...
    def _extracted_from_user_data_insert_3(self, mob, otp):
        ...
        h_user = HHIUser.objects.get(user=user)
        if(h_user.hhi_user == False):
...
class HHIUserView(viewsets.ModelViewSet):
    queryset = HHIUser.objects.all()
    serializer_class = HHIUserSerializer
    permission_classes=[IsAuthenticatedUser]
    pagination_class = StandardResultsSetPagination
    filter_backends = [DjangoFilterBackend,filters.SearchFilter]
    filter_fields = ['hhi_user','user__userId','user__email','user__address','user__mob','user__userId']
    search_fields = ['hhi_user','user__userId','user__email','user__address','user__mob','user__userId']
...
class checkCreditAccess(APIView):
    ...
    def get(self, request, user_id):
        try:
            vid_btn_hit = hhiCreditAccess.objects.filter(user=User.objects.get(id=user_id))
            print("vid_btn_hit",vid_btn_hit.first().updated_at)
            credit_time = hhiCreditTimer.objects.all().first()
...
class DoAccessCredit(APIView):
    ...
    def post(self, request):
        try:
            ...
            vid_btn_hit = hhiCreditAccess.objects.filter(user=User.objects.get(id=userId))
            print("vid_btn_hit",vid_btn_hit)
            if(vid_btn_hit.exists()):
                timediff = (dTime.now(timezone.utc) - vid_btn_hit.first().updated_at).total_seconds() / 60
                if (int(timediff) <= 40):
                    return Response({"status":"success","timeleft":timediff}, status=status.HTTP_200_OK)
            obj, created = hhiCreditAccess.objects.update_or_create(user=User.objects.get(id=userId))
...
class CountDatas(APIView):
    ...
    def get(self, request):
        ...
        users = HHIUser.objects.all().count()
...
class HHIUserInfo(APIView):
    permission_classes=[IsAuthenticatedUser]
    def get(self, request, pk, format=None):
        try:
            queryset = HHIUser.objects.get(user=pk)
            serialize = HHIUserSerializer(queryset)
...
class OrderGenerate(CsrfExemptMixin, APIView):
    ...
    def post(self, request, format=None):
        try:
            ...
            user_id = request.data.get('user')
            headers = request.META.get('HTTP_')
            with transaction.atomic():
...
def activationApi(user_id, order_num):
    url = f"http://3.108.238.214:8081/api/activate?user_id={user_id}&app_name=hhi&order_num={order_num}"
    payload={}
    headers = {
...
def bvAdditionApiNode(user_id, order_num, authorization):
    url = f"http://3.108.238.214:8081/api/bvaddition?user_id={user_id}&order_num={order_num}&app_name=hhi"
    payload={}
    headers = {
    'Cookie': 'csrftoken=EUQ1DoHGzmryjVtkrFagOe7X9VbmYDz37CLuvW2AQHq6VWgfL8fj9LzjnZVo0yZI',
...
def BillsGeneration(request, order_id):
    import pdfkit
    try:
        url = f"{getHost(request)}/hhi/api/{order_id}/bill/"
        options = {
            'page-size': 'A4',
            'encoding': "UTF-8",
            'dpi':400,
        ...
        pdfkit.from_url(url, f"hhi/media/pdfFiles/{order_id}.pdf", options=options) # .from_url and .from_string also exist
        return Response({"status":"Ok","res":{"order":order_id,"bill":f"{getHost(request)}/hhi/media/pdfFiles/{order_id}.pdf"}})
    except Exception as e:
...
def orderBill(request, order_id):
    ...
    orderProducts = OrderProduct.objects.filter(order=orderDetails)
    context = {'order':orderDetails,"orderProducts":orderProducts}
    return HttpResponse(render_to_string('hPdfConverter.html', context))
...
def InsertTagsCSv(request):
    ...
    df = pd.read_csv('hhi/media/csv/HHI Prod List.csv')
    products = []
    for _ in range(len(df)):   
        try:
            ...
            with transaction.atomic():
                ...
                #     brand.save()
                if(_ > 814):
                    brand = Brand.objects.get(BrandName="HHI")
                    product = Product.objects.filter(productCode = df.iloc[_][3]).first()
                    if(product is None):
                        product = Product.objects.create(
                            productCode = f"HHI{_+1}",
                            brand = brand,
                            productName = df.iloc[_][1],
                            productPrice = df.iloc[_][2],
                            quantity = "NA",
                            discount = df.iloc[_][3],
                            businessVolume = df.iloc[_][4],
                            status = False,
                        )
                        product.save()
