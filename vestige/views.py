...
from ecomApp.views import sendotp, presignup_sendopt, UserOtp, FinalRegistration
...
class IsAuthenticatedUser(BasePermission):
    def has_permission(self, request, view):
        if request.method in ['GET','POST', 'PUT', 'PATCH', 'DELETE']:
            ...
            return bool(
                authenticate
                and VestigeUser.objects.filter(
                    user=request.user, vestige_user=True
                ).exists()
            )
        return True  # Allow other HTTP methods without checks
...
class UserAuthentication(APIView):
    def post(self, request, *args, **kwargs):
        ...
        except VestigeUser.DoesNotExist:
    ...
    def _extracted_from_post_7(self, mob, user_mob_id):
        vestige_user = VestigeUser.objects.get(mob=mob)
        if(vestige_user.vestige_user == False):
        ...
        if(vestige_user.user_mob_id != user_mob_id):
        ...
        if mob == "+916370067211" or vestige_user.user.is_staff == True:
            return Response({"status":"ok","message":"You are a staff user, provide your potp"}, status=status.HTTP_200_OK)
        sendotp(mob, vestige_user.user)
    ...
    def post(self, request, form=None):
        try:
            with transaction.atomic():
                ...
                if(mob is None or len(mob) < 12):
                    return Response({"status": "failed","message":"Invalid mobile number!"}, status=status.HTTP_400_BAD_REQUEST)
                vestige_user = VestigeUser.objects.filter(user_mob_id=user_mob_id)
                AddLogs(None, 'UserRegistration', f"vestige_user - {vestige_user}, SignupData -{signup_data}")
                if (len(vestige_user) > 0 and vestige_user.exists()):
                ...
                if user_exist := User.objects.filter(mob=mob).exists():
                    if vestige_user := VestigeUser.objects.filter(user= User.objects.get(mob=mob)).exists():
...
class verifyOtp(APIView):
    ...
    def new_user_registration(self, mob, otp):
        ...
        if(check_pre_signup.exists()):
            return FinalRegistration(PreSignUP,check_pre_signup.first().signup_token,VestigeUser,AddLogs)
    ...
    def _extracted_from_user_data_insert_3(self, mob, otp):
        ...
        v_user = VestigeUser.objects.get(user=user)
        if(v_user.vestige_user == False):
...
class VestigeUserView(viewsets.ModelViewSet):
    queryset = VestigeUser.objects.all()
    serializer_class = vestigeUserSerializer
    permission_classes = [IsAuthenticatedUser]
    pagination_class = StandardResultsSetPagination
    filter_backends = [DjangoFilterBackend,filters.SearchFilter]
    filter_fields = ['vestige_user','user__userId','user__email','user__address','user__mob','user__userId']
    search_fields = ['vestige_user','user__userId','user__email','user__address','user__mob','user__userId']

class VestigeUserDetails(CsrfExemptMixin, APIView):
    def get(self, request, *args, **kwargs):
        VestigeUsers = VestigeUser.objects.all()
        print(VestigeUsers)
        # users = [vuser.user for vuser in VestigeUsers]
        # serializer = UserSerializer(users, many=True)
        # print(users)
        # page_number = self.request.query_params.get('page_number ', 1)
        # page_size = self.request.query_params.get('page_size ', 10)
        # paginator = Paginator(users , page_size)
        # serializer = UserSerializer(paginator(page_number), many=True, context={'request':request})
...
class checkCreditAccess(APIView):
    ...
    def get(self, request, user_id):
        try:
            vid_btn_hit = VestigeCreditAccess.objects.filter(user=User.objects.get(id=user_id))
            print("vid_btn_hit",vid_btn_hit.first().updated_at)
            credit_time = VestigeCreditTimer.objects.all().first()
...
class DoAccessCredit(APIView):
    ...
    def post(self, request):
        try:
            ...
            vid_btn_hit = VestigeCreditAccess.objects.filter(user=User.objects.get(id=userId))
            print("vid_btn_hit",vid_btn_hit)
            if(vid_btn_hit.exists()):
                timediff = (dTime.now(timezone.utc) - vid_btn_hit.first().updated_at).total_seconds() / 60
                if (int(timediff) <= 40):
                    return Response({"status":"success","timeleft":timediff}, status=status.HTTP_200_OK)
            obj, created = VestigeCreditAccess.objects.update_or_create(user=User.objects.get(id=userId))
...
class CountDatas(APIView):
    ...
    def get(self, request):
        ...
        users = VestigeUser.objects.all().count()
...
class VestigeUserInfo(APIView):
    permission_classes = [IsAuthenticatedUser]
    def get(self, request, pk, format=None):
        try:
            queryset = VestigeUser.objects.get(user=pk)
            serialize = vestigeUserSerializer(queryset)
...
class OrderGenerate(CsrfExemptMixin, APIView):
    ...
    def _extracted_from_post_7(self, serializer, user_id, authorization):
        ...
        res_serializer = OrderSerializer(res)
...
def activationApi(user_id, order_num):
    url = f"http://3.108.238.214:8081/api/activate?user_id={user_id}&app_name=vestige&order_num={order_num}"
    payload={}
    headers = {
...
def bvAdditionApiNode(user_id, order_num, authorization):
    url = f"http://3.108.238.214:8081/api/bvaddition?user_id={user_id}&order_num={order_num}&app_name=vestige"
    payload={}
    headers = {
    'Cookie': 'csrftoken=EUQ1DoHGzmryjVtkrFagOe7X9VbmYDz37CLuvW2AQHq6VWgfL8fj9LzjnZVo0yZI',
...
def BillsGeneration(request, order_id):
    import pdfkit
    try:
        url = f"{getHost(request)}/vestige/api/{order_id}/bill/"
        options = {
            'page-size': 'A4',
            'encoding': "UTF-8",
            'dpi':400,
        ...
        pdfkit.from_url(url, f"vestige/media/pdfFiles/{order_id}.pdf", options=options) # .from_url and .from_string also exist
        return Response({"status":"Ok","res":{"order":order_id,"bill":f"{getHost(request)}/vestige/media/pdfFiles/{order_id}.pdf"}})
    except Exception as e:
...
def orderBill(request, order_id):
    ...
    orderProducts = OrderProduct.objects.filter(order=orderDetails)
    context = {'order':orderDetails,"orderProducts":orderProducts}
    return HttpResponse(render_to_string('vPdfConverter.html', context))
...
def InsertTagsCSv(request):
    try:
        df = pd.read_csv('vestige/media/csv/vastige.csv')
    except Exception as e:
        print(e)
    for _ in range(len(df)):   
        try:
            ...
            with transaction.atomic():
                ...
                #     brand.save()
                if(_ > 0):
                    brand = Brand.objects.get(BrandName="Vestige")
                    product = Product.objects.filter(productCode = df.iloc[_][1]).first()
                    if(product is None):
                        product = Product.objects.create(
                            productCode = df.iloc[_][1],
                            brand = brand,
                            productName = df.iloc[_][2],
                            productPrice = int(df.iloc[_][4])+100,
                            quantity = df.iloc[_][3],
                            discount = df.iloc[_][4],
                            businessVolume = df.iloc[_][5],
                            status = True,
                        )
                        product.save()
                    cat = TagName.objects.filter(tagName=df.iloc[_][0])
                    if(len(cat)==0):
                        cat = TagName.objects.create(tagName=df.iloc[_][0])
                    else:
                        cat = cat.first()
                    if(productTagPivot.objects.filter(product=product).first() is None):
                        print("producttagpivot created: ", _)
                        
                        # subcat = TagName.objects.get(tagName=df.iloc[_][0])
                        productTagPivot.objects.create(
                            product = product,
                            category = cat,
                            subcategory = cat
                        )
                    pt = productTagPivot.objects.filter(product=product).first()
                    # pt(
                    #     category = cat,
                    #     subcategory = cat
                    # )
                    # pt.save()
                    print("producttagpivot updated: ", _, product.productCode,pt.category.tagName)
        except Exception as e:
            print(e)
