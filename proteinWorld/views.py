...
class IsAuthenticatedUser(BasePermission):
    def has_permission(self, request, view):
        if request.method in ['GET','POST', 'PUT', 'PATCH', 'DELETE']:
            ...
            return bool(
                authenticate
                and ProteinWorldUser.objects.filter(
                    user=request.user, protein_world_user=True
                ).exists()
            )
        return True  # Allow other HTTP methods without checks
...
class UserAuthentication(APIView):
    def post(self, request, *args, **kwargs):
        ...
        except ProteinWorldUser.DoesNotExist:
    ...
    def _extracted_from_post_7(self, mob, user_mob_id):
        proteinworld_user = ProteinWorldUser.objects.get(mob=mob)
        if(proteinworld_user.protein_world_user == False):
        ...
        if(proteinworld_user.user_mob_id != user_mob_id):
        ...
        if mob == "+916370067211" or proteinworld_user.user.is_staff == True:
            return Response({"status":"ok","message":"You are a staff user, provide your potp"}, status=status.HTTP_200_OK)
        sendotp(mob, proteinworld_user.user)
    ...
    def post(self, request, form=None):
        try:
            with transaction.atomic():
                ...
                if(mob is None or len(mob) < 12):
                    return Response({"status": "failed","message":"Invalid mobile number!"}, status=status.HTTP_400_BAD_REQUEST)
                protein_world_user = ProteinWorldUser.objects.filter(user_mob_id=user_mob_id)
                AddLogs(None, 'UserRegistration', f"protein_world_user - {protein_world_user}, SignupData -{signup_data}")
                if (len(protein_world_user) > 0 and protein_world_user.exists()):
                ...
                if user_exist := User.objects.filter(mob=mob).exists():
                    if protein_world_user := ProteinWorldUser.objects.filter(user= User.objects.get(mob=mob)).exists():
...
class verifyOtp(APIView):
    ...
    def new_user_registration(self, mob, otp):
        ...
        if(check_pre_signup.exists()):
            return FinalRegistration(PreSignUP,check_pre_signup.first().signup_token, ProteinWorldUser, AddLogs)
    ...
    def _extracted_from_user_data_insert_3(self, mob, otp):
        ...
        p_user = ProteinWorldUser.objects.get(user=user)
        if(p_user.protein_world_user == False):
...
class ProteinWorldUserView(viewsets.ModelViewSet):
    queryset = ProteinWorldUser.objects.all()
    serializer_class = ProteinWorldUserSerializer
    permission_classes=[IsAuthenticatedUser]
    pagination_class = StandardResultsSetPagination
    filter_backends = [DjangoFilterBackend,filters.SearchFilter]
    filter_fields = ['protein_world_user','user__userId','email','address','mob']
    search_fields = ['protein_world_user','user__userId','email','address','mob']
...
class checkCreditAccess(APIView):
    ...
    def get(self, request, user_id):
        try:
            vid_btn_hit = PWCreditAccess.objects.filter(user=User.objects.get(id=user_id))
            print("vid_btn_hit",vid_btn_hit.first().updated_at)
            credit_time = proteinWorldCreditTimer.objects.all().first()
...
class CountDatas(APIView):
    ...
    def get(self, request):
        ...
        users = ProteinWorldUser.objects.all().count()
...
class ProteinWorldUserInfo(APIView):
    permission_classes=[IsAuthenticatedUser]
    def get(self, request, pk, format=None):
        try:
            queryset = ProteinWorldUser.objects.get(user=pk)
            serialize = ProteinWorldUserSerializer(queryset)
...
def activationApi(user_id, order_num):
    url = f"http://3.108.238.214:8081/api/activate?user_id={user_id}&app_name=proteinWorld&order_num={order_num}"
    payload={}
    headers = {
...
def bvAdditionApiNode(user_id, order_num, authorization):
    url = f"http://localhost:8081/api/bvaddition?user_id={user_id}&order_num={order_num}&app_name=proteinWorld"
    payload={}
    headers = {
    'Cookie': 'csrftoken=EUQ1DoHGzmryjVtkrFagOe7X9VbmYDz37CLuvW2AQHq6VWgfL8fj9LzjnZVo0yZI',
...
def BillsGeneration(request, order_id):
    import pdfkit
    try:
        url = f"{getHost(request)}/protein-world/api/{order_id}/bill/"
        options = {
            'page-size': 'A4',
            'encoding': "UTF-8",
            'dpi':400,
        ...
        pdfkit.from_url(url, f"proteinWorld/media/pdfFiles/{order_id}.pdf", options=options) # .from_url and .from_string also exist
        return Response({"status":"Ok","res":{"order":order_id,"bill":f"{getHost(request)}/proteinWorld/media/pdfFiles/{order_id}.pdf"}})
    except Exception as e:
...
def orderBill(request, order_id):
    ...
    orderProducts = OrderProduct.objects.filter(order=orderDetails)
    context = {'order':orderDetails,"orderProducts":orderProducts}
    return HttpResponse(render_to_string('pPdfConverter.html', context))
